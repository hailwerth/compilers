...
(* Define e1 to represent:
   ...
   ( (fun f(x:int->int):bool = x(0)<1) 
     (fun f(x:int):int = x+x) )
*)
val e1= ApplyExpr(FunExpr("f","x",Bool,Bool,
                          ApplyExpr(VarExpr("f"),VarExpr("x"))),
                  ApplyExpr(FunExpr("f","x",Arrow(Int,Int),Bool,
                                    LessExpr(ApplyExpr(VarExpr("x"),IntExpr(0)),
                                             IntExpr(1))),
                            FunExpr("f","x",Int,Int,
                                    PlusExpr(VarExpr("x"),VarExpr("x")))));

(* Define e2 to represent:
   fun f(x:int):int->int->int =
     fun f(y:int):int->int =
       fun f(z:int):int = x+y+z
*)
val e2 = FunExpr("f","x",Int,Arrow(Int,Arrow(Int,Int)),
                 FunExpr("f","y",Int,Arrow(Int,Int),
                         FunExpr("f","z",Int,Int,
